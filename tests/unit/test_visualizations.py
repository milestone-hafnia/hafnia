...
def test_mask_region(compare_to_expected_image: Callable, dataset_name: str):
    sample = helper_testing.get_sample_micro_hafnia_dataset(
        dataset_name=dataset_name, force_update=False
    )
...
def test_polygon_to_bitmask_conversion(compare_to_expected_image: Callable):
    sample = helper_testing.get_sample_micro_hafnia_dataset(
        dataset_name="micro-tiny-dataset", force_update=False
    )
    image = sample.read_image()
    annotations = sample.get_annotations()
    ...
    assert len(polygons) > 0, (
        "There should be at least one Polygon annotation in the sample to test mask conversion."
    )
    for polygon in polygons:
        bitmask = polygon.to_bitmask(
            img_height=image.shape[0], img_width=image.shape[1]
        )
        bitmasks.append(bitmask)
        mask_from_polygon = polygon.to_mask(
            img_height=image.shape[0], img_width=image.shape[1], use_coco_utils=True
        )
        mask_from_bitmask = bitmask.to_mask(
            img_height=image.shape[0], img_width=image.shape[1]
        )
        assert np.array_equal(mask_from_polygon, mask_from_bitmask), (
            "Masks from Polygon and Bitmask should match."
        )
