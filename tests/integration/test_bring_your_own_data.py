...
def test_bring_your_own_data():
    ...
    user_dataset.info.version = "1.0.0"
...
def test_remove_leftover_integration_test_datasets():
    ...
    delete_integration_test_datasets = []
    for dataset_response in dataset_responses:
        ...
        delete_integration_test_datasets.append(_dataset_name)

    if len(delete_integration_test_datasets) > 0:
        delete_cmds = [f"hafnia dataset delete {_name} --no-interactive" for _name in delete_integration_test_datasets]
        pytest.fail(
            f"Found ({len(delete_integration_test_datasets)}) leftover integration test datasets: "
            f"{delete_integration_test_datasets}. "
